...
- **Deployment:** Dockerfile lives at the **repo root** (`/Dockerfile`), not inside `server/`. Reason: the Coolify MCP we use to provision the app cannot set "Base Directory", so Coolify clones the whole repo and expects the Dockerfile at the root. The Dockerfile just references `server/` from the root build context — all backend code still lives here. The mobile app is not deployed; it's built and shipped to the app stores separately.

The DB server itself (Postgres container, managed instance, whatever) is **separate from this image** — the API container only runs the Node/Hono process and connects out via `DATABASE_URL`.
